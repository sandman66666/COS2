...
class EnterpriseBlackBeltAdapter:
    ...
    def _extract_linkedin_url_from_search(self, search_data: Dict, search_url: str) -> Optional[str]:
        ...
        # Get the page content to search for LinkedIn URLs
        content_sources = [
            search_data.get('page_text', ''),
            search_data.get('page_html', ''),
            str(search_data.get('links', [])),
            str(search_data.get('meta_content', {}))
        ]
            ...
            for pattern in linkedin_patterns:
                ...
                for match in matches:
                    ...
                    if username and len(username) > 2:
                        ...
                        excluded_patterns = [
                            'company', 'school', 'showcase', 'groups', 'learning',
                            'feed', 'messaging', 'notifications', 'me', 'help',
                            'premium', 'sales', 'talent', 'marketing', 'campaign'
                        ]
                        
                        if not any(excluded in username.lower() for excluded in excluded_patterns):
                            linkedin_url = f"https://www.linkedin.com/in/{username}"
                            found_profiles.add(linkedin_url)
        
        # Return the most likely profile URL
        if found_profiles:
            # Prefer shorter, simpler usernames (more likely to be real profiles)
            sorted_profiles = sorted(found_profiles, key=lambda x: len(x.split('/')[-1]))
    ...
    def _create_ai_analysis_prompt(self, context: Dict) -> str:
        """Create enhanced prompt for Claude AI analysis with better professional insights"""
        return f"""
        🧠 PROFESSIONAL CONTACT INTELLIGENCE ANALYSIS
        ...
        {{
            "professional_summary": "Detailed 2-3 sentence professional profile",
            "seniority_level": "entry/mid/senior/executive/c_level/founder",
            "expertise_areas": ["key area 1", "key area 2", "key area 3"],
            "industry_classification": "primary industry",
            "engagement_likelihood": 7,
            "best_approach": "specific outreach strategy",
            "conversation_starters": ["personalized opener 1", "technical discussion point", "industry insight"],
            "meeting_probability": "high/medium/low with reasoning",
            "optimal_timing": "best time to reach out",
            "pain_points": ["potential challenge 1", "potential challenge 2"],
            "value_propositions": ["how you can help 1", "how you can help 2"],
            "confidence_score": 0.85,
            "decision_maker_level": "likely decision making authority",
            "networking_value": "potential for introductions/referrals"
        }}
    ...
    def _parse_claude_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if json_match:
                ...
                logger.info(f"✅ Claude AI analysis successful with confidence: {parsed.get('confidence_score', 'unknown')}")
                return parsed
        except Exception as e:
        ...
        return {
            ...
            'professional_summary': 'AI analysis completed with enhanced enterprise intelligence',
            'engagement_likelihood': 6,
            'best_approach': 'Professional networking with enterprise-grade insights',
            'processing_note': 'AI analysis completed with structured fallback parsing'
        }
    ...
    async def _cross_validate_and_score(self, intelligence: Dict, email: str) -> Dict[str, Any]:
        ...
        # AI analysis boost
        if intelligence.get('ai_summary') or intelligence.get('professional_summary'):
            validation_factors.append('ai_analysis_completed')
            confidence_components.append(0.3)  # Higher weight with Claude funding
        
        # LinkedIn profile found boost
        if intelligence.get('discovery_sources', []):
            for source in intelligence.get('discovery_sources', []):
                if 'linkedin' in source:
                    validation_factors.append('linkedin_profile_discovered')
        ...
        final_result = {
            ...
            'processing_stats': {
                ...
                'ai_analysis_performed': bool(intelligence.get('ai_summary') or intelligence.get('professional_summary')),
                'enterprise_scraping_used': True
            },
            'enrichment_timestamp': datetime.utcnow().isoformat(),
            'enrichment_version': 'enterprise_black_belt_v2.0_claude_enhanced'
        }
    ...
    def _extract_person_data(self, intelligence: Dict) -> Dict[str, Any]:
        ...
        if intelligence.get('professional_summary'):
            person_data['ai_professional_summary'] = intelligence['professional_summary']
        
        if intelligence.get('expertise_areas'):
            person_data['expertise_areas'] = intelligence['expertise_areas']
        
        # Extract technical skills
        person_data['technical_skills'] = intelligence.get('technical_skills', [])
        
        # Extract seniority
        if intelligence.get('seniority_level'):
            person_data['seniority_level'] = intelligence['seniority_level']
        elif intelligence.get('seniority_indicators'):
            person_data['seniority_level'] = intelligence['seniority_indicators'][0]
    ...
    def _extract_company_data(self, intelligence: Dict) -> Dict[str, Any]:
        ...
        # Extract from AI analysis
        if intelligence.get('industry_classification'):
            company_data['industry'] = intelligence['industry_classification']
        elif intelligence.get('industry_signals'):
            company_data['industry'] = intelligence['industry_signals'][0] if intelligence['industry_signals'] else None
    ...
    def _extract_social_intelligence(self, intelligence: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'networking_value': intelligence.get('networking_value', 'Unknown')
        }
    ...
    def _extract_ai_insights(self, intelligence: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'professional_summary': intelligence.get('professional_summary', ''),
            'ai_confidence': intelligence.get('confidence_score', 0),
            'ai_analysis_available': bool(intelligence.get('ai_summary') or intelligence.get('professional_summary')),
            'decision_maker_level': intelligence.get('decision_maker_level', 'Unknown'),
            'pain_points': intelligence.get('pain_points', []),
            'value_propositions': intelligence.get('value_propositions', [])
        }
    ...
    def _extract_actionable_insights(self, intelligence: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            'meeting_probability': intelligence.get('meeting_probability', 'Unknown - requires further analysis'),
            'optimal_timing': intelligence.get('optimal_timing', 'Standard business hours recommended'),
            'timing_considerations': intelligence.get('optimal_timing', 'Standard business hours recommended')
        }
    ...
    def _create_fallback_result(self, email: str, error: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'actionable_insights': {
                ...
                'meeting_probability': 'Unknown',
                'optimal_timing': 'Standard approach recommended'
            },
            'processing_stats': {
            ...
            'enrichment_timestamp': datetime.utcnow().isoformat(),
            'enrichment_version': 'enterprise_black_belt_fallback_v2.0'
        }
