...
# Export a simple claude client for other modules to use
def get_claude_client():
    """Get a Claude client instance for use by other modules"""
    api_key = os.getenv('ANTHROPIC_API_KEY')
    if not api_key:
        # Fallback to config if env var not set
        try:
            ...
            logger.warning("No Claude API key found - tactical alerts may not work")
            return None
    
    return anthropic.Anthropic(api_key=api_key)

# Create a global claude client instance
